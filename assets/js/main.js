...
const api_key = "fc382da80cfae9504de2157e3282432e";

navigator.geolocation.getCurrentPosition((position) => {
        let lat = position.coords.latitude;
        let lon = position.coords.longitude; 
        gertCurrentLocationWeather(lat, lon)    
    }, 
    (err) => {
        if (err.code === 1) {
...
function gertCurrentLocationWeather(lat, lon) {
...
function getCityWeather(cityName) {
    ...
    weatherIcon.src="./assets/image/search-icon.svg";
...
function displayWeather(data) {
    ...
    weatherIcon.src=`./assets/image/${icon}.svg`;
